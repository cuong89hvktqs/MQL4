...
input bool inpTrailing = true;// Cho phep trailing stop:
input double inpDiemChoPhepTrailing=20;// Diem cho phep trailing stop (pips):
input int inpTongLenhToiDa=4;// Tong lenh toi da:
input double inpStartTime=3;// Thoi Gian Bat Dau vao lenh (Server time):
...
void OnTick()
{
    ...
    Comment("Tong lenh: ",glbTongLenhDangVao);
  ...
  // Print("Vong lap moi:",BytesToRead);
   for(int i =0   ; i <BytesToRead-1 ; i++)// tim ra vi tri nen cuoi cung
     ...
     pos--;
    Print("pos=",pos," time: ",TimeToString(data[pos][0]), " clode price: ",data[pos][1], " Open h1-1: ",iOpen(Symbol(),PERIOD_H1,1));
    Print("Open h1-0: ",iOpen(Symbol(),PERIOD_H1,0)," Close h1-0:",iClose(Symbol(),PERIOD_H1,0));
//********************************
               ...
               if(AccountFreeMarginCheck(Symbol(),OP_BUY,FunGetLots())>0)
                 {
                     ...
                     if(FunKiemTraGioVaoLenh()==false) {Comment("Khong vao lenh BUY do chua toi gio vao lenh");return; }  
