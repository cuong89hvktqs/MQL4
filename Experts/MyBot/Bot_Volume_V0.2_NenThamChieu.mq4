...
input ENUM_TIMEFRAMES inpKhungLayGiaThamChieu=PERIOD_D1;// Khung lay gia dong cua tham chieu:
input int inpIndexNenThamChieu=1;//Index nen tham chieu:
input double inpLots=2;// Khoi luong giao dich:
...
input bool  inpChoPhepDichHoaVon=true;// CHo phep dich hoa von:
input double inpDiemCHoPhepoDichHoaVon=30;// Diem cho phep dich hoa von (pips):
...
string glbComment;
ENUM_XU_HUONG glbXuHuongKhungLon;
double glbGiaThamChieu=0;
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   FunKhoiTaoBoLenh();
    ...
    FunTimGioNgoaiLe();
    FunXacDinhKieuGiaoDichVaGiaThamChieu();
    FunMakeLine(glbGiaThamChieu);
...
void OnTick()
{
    if(FunKiemTraKhungThamChieuSangNenMoi()==true)
    {
        FunXacDinhKieuGiaoDichVaGiaThamChieu();
        ChartRedraw();
        FunMakeLine(glbGiaThamChieu);
   ...
   glbComment+="\nXu huong khung lon:"+FunTenXuHuongKhungLon();
   if(glbGiaThamChieu>Open[0] && glbXuHuongKhungLon==ENM_XU_HUONG_TANG)
   {
      glbComment+="\nBUY - "+DoubleToString( glbGiaThamChieu,Digits);
   }
   else if(glbGiaThamChieu<Open[0] && glbXuHuongKhungLon==ENM_XU_HUONG_GIAM)glbComment+="\nSELL - "+DoubleToString( glbGiaThamChieu,Digits);
   else glbComment+="\nXu huong khung lon chua xac dinh";
   Comment(glbComment);
   ...
   if(!FunKiemTraSangNenMoiChua()==true)// Chua sang nen moi
    ...
    if(iVolume(Symbol(),PERIOD_H1,0)>iVolume(Symbol(),PERIOD_H1,1)) //changed from h4 to h1        // Khoi luong giao dich hiện tại H1 nhỏ hơn trước đó tiếp tục
      return;
   if(MarketInfo(Symbol(),MODE_SPREAD)>inpSlippage)
      return;

   if(glbGiaThamChieu>0)
   {

      if(glbTongLenhDangVao<inpTongLenhToiDa)//Tong so lenh buy va sell dang nho hon 70
      {
         if(glbGiaThamChieu>Open[0]&& glbXuHuongKhungLon==ENM_XU_HUONG_TANG)// Gia mở cua duoi duong line canh buy
            if(FunIsBuyPinbar())// xuat hien pinbar buy, buy
            {
               double BuySL=NormalizeDouble(Ask - inpSL*glbMyPoint,Digits);
               double BuyTP=NormalizeDouble(Ask + inpTP*glbMyPoint,Digits);
         ...
         if(glbGiaThamChieu<Open[0]&&glbXuHuongKhungLon==ENM_XU_HUONG_GIAM)
            if(FunIsSellPinbar())
            {
               double SellSL=NormalizeDouble(Bid + inpSL*glbMyPoint,Digits);
               double SellTP=NormalizeDouble(Bid - inpTP*glbMyPoint,Digits);
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//**********************************QUAN LY NEN THAM CHIEU ****************************************//
void FunXacDinhKieuGiaoDichVaGiaThamChieu()
{
    if(FunNenThamChieuLaBuyPinbar() || FunnNenThamChieuLaNenBuyThanLon())
    {
        glbGiaThamChieu=iClose(Symbol(),inpKhungLayGiaThamChieu,1);
        glbXuHuongKhungLon=ENM_XU_HUONG_TANG;
        return;
    }
    if(FunNenThamChieuLaSellPinbar() || FunnNenThamChieuLaNenSellThanLon())
    {
        glbGiaThamChieu=iClose(Symbol(),inpKhungLayGiaThamChieu,1);
        glbXuHuongKhungLon=ENM_XU_HUONG_GIAM;
        return;
    }
    glbGiaThamChieu=0;
    glbXuHuongKhungLon=ENM_KHONG_XAC_DINH;

}
double FunTinhTrungBinhGiaNenThamChieu(int SoNem=4)
{
   double sum=0;
   double rangeSerie[4];
   ...
   while(i<SoNem)
     {
      ...
      if(iTime(Symbol(),inpKhungLayGiaThamChieu,ind))//(TimeDayOfWeek(Time[ind])!=0)
        {
         sum+=iHigh(Symbol(),inpKhungLayGiaThamChieu,ind)-iLow(Symbol(),inpKhungLayGiaThamChieu,ind);//make summation
         i++;
        }
      ind++;
      //i++;
...
//Comment(sum/4.0);
   return (sum/(float)SoNem);//make average, don't count min and max, this is why I divide by 4 and not by 6
}

bool FunNenThamChieuLaSellPinbar()
  {
//start of declarations
   double actOp,actCl,actHi,actLo,preHi,preLo,preCl,preOp,actRange,preRange,actLowerPart, actLowerPart1;
   actOp=iOpen(Symbol(),inpKhungLayGiaThamChieu,1);
   actCl=iClose(Symbol(),inpKhungLayGiaThamChieu,1);
   actHi=iHigh(Symbol(),inpKhungLayGiaThamChieu,1);
   actLo=iLow(Symbol(),inpKhungLayGiaThamChieu,1);
   preOp=iOpen(Symbol(),inpKhungLayGiaThamChieu,1);
   preCl=iClose(Symbol(),inpKhungLayGiaThamChieu,1);
   preHi=iHigh(Symbol(),inpKhungLayGiaThamChieu,1);
   preLo=iLow(Symbol(),inpKhungLayGiaThamChieu,1);
...
//start function body
   ...
   double dayRange=FunTinhTrungBinhGiaNenThamChieu(4);//Trung binh cua 4 nen truoc do (high-low)
...
//(actLo>(preLo+preRange/3.0))&& //Low of PB is NOT lower than 1/2 of previous Bar
   ...
   {
      double ArrHigh[3];
      ArrHigh[0]=iHigh(Symbol(),inpKhungLayGiaThamChieu,2);
      ArrHigh[1]=iHigh(Symbol(),inpKhungLayGiaThamChieu,3);
      ArrHigh[2]= iHigh(Symbol(),inpKhungLayGiaThamChieu,4);
      if(ArrHigh[ArrayMaximum(ArrHigh,WHOLE_ARRAY,0)]<iHigh(Symbol(),inpKhungLayGiaThamChieu,1))
         return (true);
   }
   return false;
  }//------------END FUNCTION-------------

bool FunNenThamChieuLaBuyPinbar()
  {
//start of declarations
...
//end of declaratins
//start function body
   double dayRange=FunTinhTrungBinhGiaNenThamChieu(4);
...
//(actHi<(preHi-preRange*0.3))&& //High of PB is NOT higher than 1/2 of previous Bar
      (actLo+actRange*0.25<preLo)) //Nose of the PB is at least 1/3 lower than previous bar
     {
      ...
      double ArrLow[3];
      ArrLow[0]=iLow(Symbol(),inpKhungLayGiaThamChieu,2);
      ArrLow[1]=iLow(Symbol(),inpKhungLayGiaThamChieu,3);
      ArrLow[2]= iLow(Symbol(),inpKhungLayGiaThamChieu,4);   
      if(ArrLow[ArrayMinimum(ArrLow,WHOLE_ARRAY,0)]>iLow(Symbol(),inpKhungLayGiaThamChieu,1))
         return (true);
     }
   return(false);

  }//------------END FUNCTION-------------
bool FunnNenThamChieuLaNenBuyThanLon()
{
   double actOp,actCl,actHi,actLo;
   actOp=iOpen(Symbol(),inpKhungLayGiaThamChieu,1);
   actCl=iClose(Symbol(),inpKhungLayGiaThamChieu,1);
   actHi=iHigh(Symbol(),inpKhungLayGiaThamChieu,1);
   actLo=iLow(Symbol(),inpKhungLayGiaThamChieu,1);
   double bodyCandel=actCl-actOp;
   double MidBodyCandel=(actCl+actOp)/2;
   double MAvalue=iMA(Symbol(),inpKhungLayGiaThamChieu,21,0,MODE_EMA,PRICE_CLOSE,1);
   if (bodyCandel>0)
   {
      if(bodyCandel>0.618*(actHi-actLo) && MidBodyCandel>MAvalue)
         return true;
   }
   return false;
}
bool FunnNenThamChieuLaNenSellThanLon()
{
   double actOp,actCl,actHi,actLo;
   actOp=iOpen(Symbol(),inpKhungLayGiaThamChieu,1);
   actCl=iClose(Symbol(),inpKhungLayGiaThamChieu,1);
   ...
   if (bodyCandel<0)
   {
      if(MathAbs(bodyCandel)>0.618*(actHi-actLo) && MidBodyCandel<MAvalue)
         return true;
   }
   return false;
}
//+------------------------------------------------------------------+
string FunTenXuHuongKhungLon()
{
   string s;
   switch (glbXuHuongKhungLon)
   {
   case ENM_XU_HUONG_TANG:
      s="XU HUONG TANG";
      break;
   case ENM_XU_HUONG_GIAM:
      s="XU HUONG GIAM";
      break;
   default:
      s="KHONG XAC DINH";
...
bool FunKiemTraKhungThamChieuSangNenMoi()
{
    static datetime _LastBarKhungThamChieu;
   datetime currBarKhungThamChieu=iTime(Symbol(),inpKhungLayGiaThamChieu,0);//  Time[0];//iTime(OrderSymbol(),0,0);
   if(_LastBarKhungThamChieu!=currBarKhungThamChieu)
   {
      _LastBarKhungThamChieu=currBarKhungThamChieu;
      return true;
   }
   else return false;
}
   ...
   if(inpRisk!=0)
     {
        ...
        lot=riskmoney/(tickevalue*inpSL*glbMyPoint/Point());
        if(lot<minlot)
            lot=minlot;
        if(lot>maxlot)
            lot=maxlot;
...
void FunTraiLingStopTicket(int ticket)
{
      ...
      {    
        ...
        if(OrderType()==OP_BUY)
        {
            SoPipLoiLo=(Bid -OrderOpenPrice())/glbMyPoint;
        }
        else if(OrderType()==OP_SELL)
        {
            SoPipLoiLo=(OrderOpenPrice()-Ask)/glbMyPoint;
        }
         ...
         if (SoPipLoiLo>DiemTrailingStop)
         {
           // Print("So pip loi lo: ",SoPipLoiLo," Diem trailing: ",DiemTrailingStop);
            ...
            if(OrderType()==1)//lệnh sell
            {
               ...
               StopLoss=Bid+ DiemTrailingStop*glbMyPoint;
