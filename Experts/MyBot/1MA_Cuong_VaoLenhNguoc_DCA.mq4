...
input double inpKhoangCachNhoiLenhDauTien=20;//Khoang cach nhoi lenh dau tien (pips):
input double inpHeSoCongKhoangCach=5;// He so cong khoang cach nhoi lenh:
...
//input int inpSoPipDichHoaVon=100;// So pip dich hoa von (=0: khong dich hoa von):
//input int inpSoPipTrailing=150;// So pip trailing stop (=0: khong trailing stop):
input bool inpChoPhepDongLenhKhiGiaCatMa=true;// Chp phep dong lenh khi gia cat MA:
input string str0="THAM SO DUONG MA ";//CAI DAT THAM SO MA:
input ENUM_TIMEFRAMES inpTFKhung=PERIOD_CURRENT;//Timeframe tinh MA:
input int inpMAPeriodNgan=50;// MA Period:
input ENUM_MA_METHOD inpMAMethodNgan= MODE_EMA;//MA  Method :
input ENUM_APPLIED_PRICE inpAppliedPriceNgan=PRICE_CLOSE;// MA  applied price : 
...
input double inpKhungGioBatDauMoLenh=17;// Khung gio bat dau mo lenh:
input double inpKhungGioKetThucMoLenh=20;// Khung gio Ket thuc mo lenh:
...
int glbTapLenh[100];
int glbTongLenh=0;
datetime glbNgayVaoLenhHienTai;
...
void OnTick()
{
        ...
        if(FunKiemTraGioVaoLenh()==true)
        {
            ...
            if(DieuKienVaoLenh>=0)
            {
                ...
                if(ticket>0)
                {
                     ...
                     glbNgayVaoLenhHienTai=FunTinhNgayVaoLenhHienTai(TimeCurrent());
                     Print("Ngay vao lenh: ",glbNgayVaoLenhHienTai);
                }
            }
        }
   ...
   {
      if(inpChoPhepDongLenhKhiGiaCatMa) FunDongLenhKhiCatMA();
...
void FunKiemTraNhoiLenh()
{
   ...
   {  
      ...
      if(Ask<=glbGiaNhoiLenhTiepTheo && FunKiemTraDieuKienVaoLenh()==0 && FunTinhNgayVaoLenhHienTai(TimeCurrent())>glbNgayVaoLenhHienTai)
   ...
   else
   {
      ...
      if(Bid>=glbGiaNhoiLenhTiepTheo && FunKiemTraDieuKienVaoLenh()==1 && FunTinhNgayVaoLenhHienTai(TimeCurrent())>glbNgayVaoLenhHienTai)
...
datetime FunTinhNgayVaoLenhHienTai(datetime time)
{
   string s=IntegerToString(TimeYear(time))+"."+IntegerToString(TimeMonth(time))+"."+IntegerToString(TimeDay(time));
...
double FunTinhGiaCachNhoiLenhTiepTheo()
{
   double BuocLenhTiepTheo;
    ...
    BuocLenhTiepTheo=inpKhoangCachNhoiLenhDauTien+inpHeSoCongKhoangCach*(glbTongLenh-1);
...
int FunKiemTraDieuKienVaoLenh()//0: Buy, 1: sell
{
    ...
    double KhoangCachPips=(Bid-MAValue)/glbMyPoint;
    if(MathAbs(KhoangCachPips)>=inpKhoangCachMaKhiVaoLenh)
    {
        if(KhoangCachPips<0) return 0;// Thoa man DK de buy
        else return 1;
    }
    return -1;
}
...
{
    double MAValue=FunTinhMAValue(inpTFKhung,inpMAPeriodNgan,inpMAMethodNgan,inpAppliedPriceNgan,0);
    bool DieuKienDongLenh=(glbLoaiLenh==OP_BUY && Bid>=MAValue)||(glbLoaiLenh==OP_SELL && Bid<=MAValue);
    if(DieuKienDongLenh)
    {
        FunDongTatCaCacLenh();
    }
...
void FunTimLenhCoSo()
{
   ...
   if(glbTongLenh>0)
   {
      ...
      glbSoLotCuaLenhTiepTheo=FunTinhKhoiLuongVaoLenhTiepThep();
      OrderSelect(glbTapLenh[glbTongLenh-1],SELECT_BY_TICKET);
      glbNgayVaoLenhHienTai=FunTinhNgayVaoLenhHienTai(OrderOpenTime());
   }
}
...
void FunDongTatCaCacLenh()
{
   ...
   FunHLineDelete(0,"hLineGiaNhoiLenh");
   FunHLineDelete(0,"hLineGiaDongLenh");
}
//+------------------------------------------------------------------+
