...
input string str0="THAM SO HAI DUONG BB VA RSI ";//CAI DAT THAM SO MA:
...
input int inpCCIPeriod=14; //RSI period:
input ENUM_APPLIED_PRICE inpCCIAppliedPrice=PRICE_CLOSE;//RSI applied price:
input double inpCCIDuoi=30;// RSI can duoi:
input double inpCCITren=70;//RSI can tren:
...
input double inpKhungGioBatDauMoLenh=23;// Khung gio bat dau mo lenh (Gio san):
input double inpKhungGioKetThucMoLenh=18;// Khung gio Ket thuc mo lenh (Gio san):
...
int FunKiemTraDieuKienVaoLenh()//0: Buy, 1: sell
{
    ...
    RSIValue=iRSI(Symbol(),PERIOD_CURRENT,inpCCIPeriod,inpCCIAppliedPrice,0);
...
int FunTinHieuCatLenh()//0: CAT LENH SELL, 1 : CAT LENH BUY
{
   ...
   {
      RSIValue=iRSI(Symbol(),PERIOD_CURRENT,inpCCIPeriod,inpCCIAppliedPrice,1);
